...
def appendMovie(filename="../ml-1m/movies.dat"):
    f = open(filename, encoding='ISO-8859-1')
    network  = open("network.txt",mode='a+',encoding='utf-16')
    lines = f.readlines()
    for line in tqdm(lines):
        alist = line.strip().split("::")
        subjects = alist[2].split("|")
        for subject in subjects:
            network.write('m'+alist[0]+'@'+subject+'\n')
...
if __name__ == '__main__':
    ...
    model = DeepWalk(G, walk_length=100, num_walks=80, workers=4)
    ...
    for N in range(5,21,5):
        ...
        print("microF1:{}".format((2*microRecall*microPrecision)/(microRecall+microPrecision)))
        np.savetxt("../results/precision_list_deepwalk_opt_no_top"+str(N)+".txt", precision_list)
        np.savetxt("../results/recall_list_deepwalk_opt_no_top"+str(N)+".txt", recall_list)
